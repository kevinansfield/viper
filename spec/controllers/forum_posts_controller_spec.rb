...
describe ForumPostsController, "GET #new" do
  include ForumPostsControllerParentObjects
  define_models :stubbed
...
describe ForumPostsController, "POST #create" do
  ...
  before do
    login_as :default
    @attributes = {:body => 'foo'}
    @post = mock_model ForumPost, :new_record? => false, :errors => []
    @user.stub!(:reply_to_forum_topic).with(@topic, @attributes[:body]).and_return(@post)
  end
  ...
  describe ForumPostsController, "(successful creation)" do
    ...
    it_redirects_to { forum_topic_post_path(@forum, @topic, @post, :anchor => "forum_post_#{@post.id}") }
  end
...
describe ForumPostsController, "PUT #update" do
  ...
  before do
    @attributes = {}
    @post = forum_posts(:default)
    ForumPost.stub!(:find).with('1').and_return(@post)
  end
  ...
  describe ForumPostsController, "(successful save)" do
    ...
    it_assigns :post, :forum, :topic, :parent => lambda { @topic }, :flash => { :notice => :not_nil }
    it_redirects_to { forum_topic_path(@forum, @topic, :anchor => "forum_post_#{@post.id}") }
  end
